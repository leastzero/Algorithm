...
while jihoon_Q:
    ...
    for i in range(4):
        ...
        # 갈 수 있는 길인데 아직 안 간 길이라면
        if 0 <= xx < r and 0 <= yy < c and board[xx][yy] == '.' and visited_jihoon[xx][yy] == -1:
            # 불이 없거나 불보다 지훈이가 더 먼저 도착하는 경우라면
            if visited_fire[xx][yy] == -1 or visited_fire[xx][yy] > visited_jihoon[x][y] + 1:
